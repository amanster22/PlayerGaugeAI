...
const handleAskAI = async () => {
    ...
    try {
      // 2. Send the question to your NEW Python AI Server (Port 5000)
      ...
      // 3. Display the answer the Agent calculated
      if (data.error) {
        setResponse(`Error: ${data.error}`);
      } else {
        setResponse(data.answer || "No response generated by the Agent.");
      }

    } catch (error) {
      console.error("Error connecting to AI:", error);
      setResponse("Could not connect to the Python AI Server. Is 'python app.py' running?");
    } finally {
      setLoading(false);
    }
  };
